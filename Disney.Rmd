...
disney_short%>%write_csv("disney_short_17_10_21.csv")
```
...
```{r}
ts_plot(disney_short, "hours") +
  labs(x = NULL, y = NULL,
       title = "Frequency of tweets with Disney",
       subtitle = paste0(format(min(disney_short$created_at), "%d %B %Y"), " to ", format(max(disney_short$created_at),"%d %B %Y")),
       caption = "Disney tweets Data collected from Twitter") +
  theme_minimal()
```
